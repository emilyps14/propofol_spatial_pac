...
from src_loc_modules import plot_sources
    ...
    def _plot_and_save(self, reject, yl_power, yl_diff, fname, blnsave, amp_band_i,
        ...
        if reject is None:
            ...
            stc_corr = mne.SourceEstimate(toplot.T,
                                          ...
                                          tmin=0, tstep=1,
                                          subject=self.subject)
            brain = plot_sources.plot_surf(stc_corr, {'kind': 'value',
                                                      'lims': ctrl_pts},
                                           'seismic', label_func,
                                           'semi-inflated', False,
        ...
        if blnsave:
            for j,(t,level) in enumerate(zip(stc_corr.times,self.eventnames)):
                brain.set_time(t)
                plot_sources.save_surf(brain, fname, self.figures_output_path,
                                   '_{}{}'.format(j, level))
            brain.close()
    ...
    def plot_freq_profile_projections_surfaces(self,blnsave,nprofiles=2,
        ...
        for i in range(nprofiles):
            ...
            stc_proj = mne.SourceEstimate(toplot.T,
                                          ...
                                          tmin=0, tstep=1,
                                          subject=self.subject)
            brain = plot_sources.plot_surf(stc_proj, {'kind': 'value',
                                                      'lims': ctrl_pts},
                                           'seismic', label_func,
                                           'semi-inflated', False,
                                           self.blnOffscreen,subjects_dir=self.subjects_dir,
